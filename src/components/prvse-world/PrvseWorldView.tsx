...
import { createPrvseAtmosphere, type PrvseAtmosphere } from './engine/prvse-atmosphere'
...
import RuntimeSOverlay from './overlay/RuntimeSOverlay'
...
export default function PrvseWorldView() {
  ...
  const atmosphereRef = useRef<PrvseAtmosphere | null>(null)
  ...
  useEffect(() => {
    ...
    if (!ctx) {
      ...
      const atmosphere = createPrvseAtmosphere()
    ...
    function tick() {
      ...
      atmosphereRef.current?.tick(Date.now() / 1000)
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full overflow-hidden bg-[#050508]">
      ...
      {/* runtime S + CFL registry + last stand HUD */}
      {!l1Panel && !spherePanel && <RuntimeSOverlay />}
